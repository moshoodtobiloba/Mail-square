import { Mail, Zap, ShieldAlert, Plus, CheckCircle2 } from 'lucide-react';

export default function InboxesView() {
  const inboxes = [
    { email: 'hello@founder.io', type: 'Google Workspace', strength: 98, warmup: 'Active', sent: 120 },
    { email: 'sales@founder.io', type: 'Google Workspace', strength: 80, warmup: 'Active', sent: 45 },
    { email: 'noreply@founder.io', type: 'Free Gmail', strength: 40, warmup: 'Warming', sent: 5 },
  ];
  ...
  return (
    <div className="animate-in fade-in duration-500 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button className="px-4 py-2 bg-blue-600 text-white border border-blue-700 rounded-full text-sm font-medium hover:bg-blue-700 flex items-center gap-2 shadow-sm transition-all focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 cursor-pointer">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {inboxes.map((inbox, i) => (
          <div key={i} className="utility-card p-6 flex flex-col relative overflow-hidden hover:shadow-md transition-shadow">
            ...
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-medium text-gray-900 text-sm truncate max-w-[150px]">{inbox.email}</h3>
                  <p className="text-xs text-gray-500">{inbox.type}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-5 flex-grow">
               <div>
                  ...
                  <div className="flex items-center gap-1 mt-2 text-[10px] text-gray-400 font-medium">
                     {inbox.strength > 90 ? <><CheckCircle2 className="w-3 h-3 text-emerald-500"/> Optimal sending condition</> : 
                      inbox.strength > 50 ? <><Zap className="w-3 h-3 text-amber-500"/> Improving with warmup</> : 
                      <><ShieldAlert className="w-3 h-3 text-red-500"/> High risk of spam folder</>}
                  </div>
               </div>
               ...
               <div className="grid grid-cols-2 gap-3">
                 <div className="bg-gray-50 border border-gray-100 rounded-lg p-3 text-center">
                    ...
                    <p className="text-sm font-medium flex items-center justify-center gap-1.5 text-gray-900">
                      {inbox.warmup === 'Active' ? <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div> : <div className="w-1.5 h-1.5 rounded-full bg-amber-500"></div>}
                      {inbox.warmup}
                    </p>
                 </div>
                 <div className="bg-gray-50 border border-gray-100 rounded-lg p-3 text-center">
            ...
            <div className="mt-5 pt-4 border-t border-gray-100 grid grid-cols-2 gap-2">
               <button className="text-xs font-medium text-center text-gray-600 hover:text-gray-900 py-2 hover:bg-gray-50 rounded transition-colors cursor-pointer">Manage</button>
               <button className="text-xs font-medium text-center text-blue-600 hover:text-blue-700 py-2 hover:bg-blue-50 rounded transition-colors cursor-pointer">Open Inbox</button>
            </div>
          </div>
        ))}

        <button className="utility-card border-dashed border-2 border-gray-200 p-6 flex flex-col items-center justify-center text-center hover:border-gray-300 hover:bg-gray-50 transition-colors group min-h-[260px] cursor-pointer">
